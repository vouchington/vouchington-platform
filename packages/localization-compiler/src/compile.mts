...
function insertCatalog(database: DatabaseSync, catalog: LocalizationCatalog): void {
  ...
  const insertRouteSelector = database.prepare(
    'INSERT INTO route_selectors (consumer, selector_id) VALUES (?, ?)',
  )
  ...
  for (const selector of catalog.routeSelectors ?? [])
    insertRouteSelector.run(selector.consumer, selector.selectorId)
