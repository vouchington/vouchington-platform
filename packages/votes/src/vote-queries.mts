...
import { quoteSqlIdentifier } from './identifiers.mts'
...
export function insertQuery(
  ...
  const safeTable = quoteSqlIdentifier(table, 'table')
  const safeColumn = quoteSqlIdentifier(column, 'entityIdColumn')
...
export function userVotesQuery(
  ...
  if (entityIds && entityIds.length > 0) {
    query.append(safeColumn).append(sql` = ANY(${entityIds}::uuid[]) AND vote_event.`)
  }
  query.append(safeColumn)
  if (afterEntityId) query.append(sql` > ${afterEntityId} AND vote_event.`).append(safeColumn)
  query
